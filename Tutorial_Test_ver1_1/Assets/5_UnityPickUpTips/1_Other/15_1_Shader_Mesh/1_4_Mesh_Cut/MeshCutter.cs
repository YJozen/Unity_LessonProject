...
namespace BLINDED_AM_ME
{
    ...
    public class MeshCut
    {
        ...
        {
            public List<Vector3> vertices = new List<Vector3>();//頂点
            public List<Vector3> normals  = new List<Vector3>();//法線
            ...
            /// <summary>トライアングルとして3頂点を追加 ※ 頂点情報は元のメッシュからコピーする </summary>
            ...
            /// <summary> トライアングルを追加する
            /// ※ オーバーロードしている他メソッドとは異なり、引数の値で頂点（ポリゴン）を追加する </summary>
            ...
            public void AddTriangle(Vector3[] points3, Vector3[] normals3, Vector2[] uvs3, Vector3 faceNormal, int submesh) {
                ...
                // 引数で指定された法線と逆だった場合はインデックスの順番を逆順にする（面を裏返す）
        ...
        private static List<Vector3> new_vertices = new List<Vector3>();// capping stuff

        /// <summary> Cut the specified victim, blade_plane and capMaterial.
        /// （指定された「victim」をカットする。ブレード（平面）とマテリアルから切断を実行する） </summary>
        ...
        public static GameObject[] Cut(GameObject victim, Vector3 anchorPoint, Vector3 normalDirection, Material capMaterial) {
            //切断面を表す
            // set the blade relative to victim// victimから相対的な平面（ブレード）をセット
            ...
            victim_mesh = victim.GetComponent<MeshFilter>().mesh;// get the victims mesh  対象のメッシュを取得

            new_vertices.Clear(); // reset values 新しい頂点郡
            left_side.ClearAll() ;// 平面より左の頂点郡（MeshCutSide）
            right_side.ClearAll();// 平面より右の頂点郡（MeshCutSide）
            ...
            // go throught the submeshes サブメッシュの数だけループ
            for (int sub = 0; sub < victim_mesh.subMeshCount; sub++) {
                
                indices = victim_mesh.GetIndices(sub);// サブメッシュのインデックス数を取得
                ...
                for (int i = 0; i < indices.Length; i += 3) {
                    ...
                    sides[0] = blade.GetSide(victim_mesh.vertices[p1]);//Planeクラス の　GetSideで　どっちサイドか把握
                    ...
                    if (sides[0] == sides[1] && sides[0] == sides[2]) {
                        if (sides[0]) { // left side// GetSideメソッドでポジティブ（true）の場合は左側にあり
                            left_side.AddTriangle(p1, p2, p3, sub);
                        } else {
                            right_side.AddTriangle(p1, p2, p3, sub);
                        }
                    } else { // cut the triangle// そうではなく、どちらかの点が平面の反対側にある場合はカットを実行する
                        Cut_this_Face(sub, sides, p1, p2, p3);
                    }
                }
            }
            
            Material[] mats = victim.GetComponent<MeshRenderer>().sharedMaterials;// 設定されているマテリアル配列を取得
            
            if (mats[mats.Length - 1].name != capMaterial.name) { // add cap indices // 取得したマテリアル配列の最後のマテリアルが、カット面のマテリアルでない場合
                // カット面用のインデックス配列を追加？
                left_side.subIndices.Add(new List<int>());
                right_side.subIndices.Add(new List<int>());

               
                Material[] newMats = new Material[mats.Length + 1]; // カット面分増やしたマテリアル配列を準備

                mats.CopyTo(newMats, 0);// 既存のものを新しい配列にコピー

                newMats[mats.Length] = capMaterial; // 新しいマテリアル配列の最後に、カット面用マテリアルを追加

                mats = newMats;// 生成したマテリアルリストを再設定
            }

            ///    // cap the opennings// カット開始   ///
            Capping();


            // Left Mesh 左側のメッシュを生成
            ...
            GameObject leftSideObj = victim;// 右側のオブジェクトは新規作成

            GameObject rightSideObj                      = new GameObject("right side", typeof(MeshFilter), typeof(MeshRenderer), typeof(Rigidbody));//GameObject新規作成

            rightSideObj.transform.position              = victim.transform.position;
            rightSideObj.transform.rotation              = victim.transform.rotation;
            rightSideObj.GetComponent<MeshFilter>().mesh = right_HalfMesh;

            // assign mats  // 新規生成したマテリアルリストをそれぞれのオブジェクトに適用する
            leftSideObj.GetComponent<MeshRenderer>().materials  = mats;
            rightSideObj.GetComponent<MeshRenderer>().materials = mats;
            
            return new GameObject[] { leftSideObj, rightSideObj };// 左右のGameObjectの配列を返す
        }//Cut
        ...
        /// <summary> カットを実行する。ただし、実際のメッシュの操作ではなく、あくまで頂点の振り分け、事前準備としての実行 </summary>
        ...
        static void Cut_this_Face(int submesh, bool[] sides, int index1, int index2, int index3) {
            ...
            //・辺と距離から切断頂点を求める
            //Raycast を用いて切断頂点までの「距離」を求めることができました。//ただこれはスカラー値で、あくまで「距離」しか分かりません。
            ...
            //これを、分断された辺の分、つまり2辺分行うことで分断された頂点を求めることができる

            
            normalizedDistance = distance / (rightPoints[0] - leftPoints[0]).magnitude;// 見つかった交差点を、頂点間の距離で割ることで、分割点の左右の割合を算出する
            ...
            new_vertices.Add(newVertex1);// 新頂点郡に新しい頂点を追加
            ...
            new_vertices.Add(newVertex2);// 新頂点郡に新しい頂点を追加
        ...
        /// <summary> カットを実行 </summary>
        static void Capping() {
            // カット用頂点追跡リスト// 具体的には新頂点全部に対する調査を行う。その過程で調査済みをマークする目的で利用する。
            ...
            for (int i = 0; i < new_vertices.Count; i++) {
                ...
                capVertpolygon.Clear();// カット用ポリゴン配列をクリア
                ...
                while (!isDone) {
                    ...
                    //生成された重複頂点を調べてグルーピングする(Cubeを切断した場合、Cubeをぐるっと取り囲むように16個の頂点が作られます。)
                    ...
                    //つまり、入れ子のfor文が行っているのは、必ず重複頂点が生成される事実を利用して、重複頂点を結んでいくことで「角」と「腕」のカット面を意味的にふたつにグルーピングしている
