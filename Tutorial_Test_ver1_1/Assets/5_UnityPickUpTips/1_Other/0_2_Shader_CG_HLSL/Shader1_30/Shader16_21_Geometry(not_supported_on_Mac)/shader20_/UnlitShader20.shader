        ...
        Pass
        {
            ...
            void geom(triangle appdata input[3], uint pid : SV_PrimitiveID,inout TriangleStream<g2f> stream)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    //上記の　順番を変えただけ
                    v.vertex.xyz = center + rotate(v.vertex.xyz - center, (pid + _Time.y) * _RotationFactor, r3);//回転させる
                    v.vertex.xyz = center + (v.vertex.xyz - center) * (1.0 - _ScaleFactor);//中心を起点にスケールを変える
                    v.vertex.xyz += normal * _PositionFactor * abs(r3); //法線ベクトルに沿って頂点を移動　
