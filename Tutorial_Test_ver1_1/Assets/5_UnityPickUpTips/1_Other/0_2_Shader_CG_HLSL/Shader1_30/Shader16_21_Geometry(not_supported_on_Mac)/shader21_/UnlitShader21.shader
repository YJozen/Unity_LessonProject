        ...
        Pass
        {
            ...
            void geom(triangle appdata input[3], uint pid : SV_PrimitiveID,inout TriangleStream<g2f> stream)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    v.vertex.xyz = center + rotate(v.vertex.xyz - center, (pid + _Time.y) * _RotationFactor, r3);//回転　        アニメーション側で制御している値(SetFloat)を利用
                    v.vertex.xyz = center + (v.vertex.xyz - center) * (1.0 - _ScaleFactor);                      //スケール変更 　アニメーション側で制御している値(SetFloat)を利用
                    v.vertex.xyz += normal * _PositionFactor * abs(r3);                                          //法線方向に移動　アニメーション側で制御している値(SetFloat)を利用                  
                    v.vertex.y += _GravityFactor;                                                                //アニメーション側で制御している値(SetFloat)を利用　Y座標に渡して重力っぽく見せる
