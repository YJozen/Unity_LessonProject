            ...
            output frag(const v2f i)
            {
                ...
                for (int i = 0; i < 40; i++)
                {
                    ...
                    if (dist < 0.01)
                    {
                        ...
                        //「頂点⇨ライトへのベクトル」と「頂点⇨カメラへのベクトル」を足して正規化した「ハーフベクトル」というものを使います。
                        //このハーフベクトルと法線ベクトルの内積がライトの反射量を表す指標になるので、この値が一定以上のときにハイライトを描画します。
                        //ハイライトの部分を明るく不透明にするために、ハイライトとみなされる範囲のピクセルは色と不透明度の値に1を足しています
                        float highlight = dot(norm, halfDir) > 0.99 ? 1 : 0; // ハイライト  内積が　０．９９以上なら反射　として
                        fixed3 color = clamp(lerp(baseColor, rimColor, rimRate) + highlight, 0, 1); // 色 lerp ( A, B, T )入力 A と入力 B の間を入力 T によって線形補間
                        float alpha = clamp(lerp(0.2, 4, rimRate) + highlight, 0, 1); // 不透明度　clamp（In　Min　Max）   lerp
