Shader "Slime/Step3"//球の座標・半径をColliderとリンクさせる
{
    Properties {}
    SubShader
    {
        ...
        Pass
        {
            ...
            float getDistance(float3 pos)
            {
                ...
                {
                    dist = min(dist, sphereDistanceFunction(_Spheres[i], pos));//配列の中身を見て、球との距離を把握（球の半径考慮）
                }
                return dist;
            }
            ...
            output frag(const v2f i)
            {
                ...
                for (int i = 0; i < 30; i++)
                {    
                    float dist = getDistance(pos); // posと球との最短距離

                    // posと球との距離が0.001以下になったら、色と深度を書き込んで   球を表現していく   処理終了
                    if (dist < 0.001)
                    {
