Shader "Slime/Step7"//リムライト(物体の輪郭を明るく光らせる)と透明度を実装する
{
    Properties {}
    SubShader
    {
            ...
            output frag(const v2f i)
            {
                ...
                for (int i = 0; i < 40; i++)
                {
                    ...
                    if (dist < 0.01)
                    {
                        ...
                        const float rimRate = pow(1 - abs(dot(norm, rayDir)), rimPower); //輪郭らしさ＝（１－｜法線ベクトル・視線ベクトルの内積｜）＾ｋ   　 輪郭らしさの指標
                        const fixed3 rimColor = fixed3(1.5, 1.5, 1.5);// 輪郭の色

                        fixed3 color = clamp(lerp(baseColor, rimColor, rimRate), 0, 1); // 色　補間

                        //計算した輪郭らしさの指標を使って、輪郭から離れるほど透明に、輪郭に近づくほど不透明にします。
